...
struct song* read_kidi_binary(FILE* file_ptr) 
{
	...
	while(1)
	{	
		if (curr_song->num_events == curr_song->event_capacity) 
		{
			resize_song_struct(curr_song, curr_song->event_capacity * 2 );
		}
	
		struct event* evt = &(curr_song->event_array[curr_song->num_events]);
		
		read_uint16(file_ptr, &(evt->delta_time));
		read_uint8(file_ptr, &(evt->event_type));
		
		// Break if the "done" meta event is found
		if (evt->event_type == 0xFF)
		{
			break;
		}
...
struct song* read_kidi_ascii(FILE* file_ptr) 
{
	...
	while(fgets(line, 100, file_ptr) != NULL)
	{
		...
		evt->channel = atoi(tokens);
		tokens = strtok(NULL, " \t");
		
		evt->frequency = atoi(tokens);
		tokens = strtok(NULL, " \t");
		
		evt->velocity = atoi(tokens);
