...
data AssignableValue = Assignable ValidValue | UnInitialized 
data ValidValue = IntValue Integer | BoolValue Bool deriving(Eq, Show)
...
contains :: VariableName -> State -> Bool
contains varName state = length(filter ((==varName).fst) state) > 0
-------------------------------------------------------------------
...
addToState var state 
    | ((contains var state) == True) = state 
    | otherwise = state ++ [(var, UnInitialized)]
--    | otherwise = (var, UnInitialized):state
...
bind varName value state = map (\x -> if (varName == (fst x)) then (varName, value) else x) state
...
deref varName (x:xs) 
    | (fst x) == varName = if snd x == UnInitialized
                             then BoolValue False
                             else getVal (snd x)
    | xs == [] = BoolValue False
    | otherwise = deref varName xs

getVal :: AssignableValue -> ValidValue
getVal (Assignable a) = a
